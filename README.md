...
預訓練和微調（Pre-training and Fine-tuning）：BERT 首先在大規模文本語料上進行預訓練，然後在特定任務上進行微調。預訓練過程包括兩個任務，遮罩語言模型（MLM）：在預訓練過程中，BERT 隨機遮罩掉輸入文本中的一些單詞，然後要求模型預測這些被遮罩的單詞ㄤ;下一句預測（NSP）：BERT 接收成對的句子，並預測第二個句子是否是第一個句子的下一句。<br>
